...
`_SPEED` (content.py): `leaden` / `heavy` / `steady` / `hybrid` / `brisk` / `speedy` / `blinding`, slow→fast. Faster ⇒ ↑`attack_speed`/`resistance`/`move_speed`, ↓`primary_stat` (softer per-hit). `hybrid` is the neutral centre (omitted from `role_code`). Widening 3→7 took the role-code space to 1512 combos (V.32); `classify_role` ignores speed, so role *titles* are unaffected.
