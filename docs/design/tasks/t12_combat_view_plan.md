...
> **⚠️ OUT OF DATE (2026-06-22) — re-plan before `/build §T.12a`.** This doc's playback design (**§3.2 "Bars read `PieceFrame.hp` from `hp_after`", "NO re-sim"**; **§3.6 V.54 posture**; **§8 board-fidelity test**; the `combat_playback` "frames" model) was **superseded** by **B.28 + V.57 + T.37c**. The recorded event stream is **incomplete** for resource truth — registered-ability burst emits no `hp_after` beat — so bars must read the **live forward `CombatReplay` stepper** (`game/combat/replay.py`, T.37c), **not** the stream. Stream = **animation cues + action-queue projection only**. T.12a now also **depends on T.37c** (not just T.37a/b). Sections needing rewrite: §1 scope (add T.37c dep), §2 gap table, §3.2 (stepper-driven bars, drop "frames from `hp_after`"), §3.3 (bars/inspect via `CombatReplay`), §3.6 (V.54→V.57), §8 (B.28 guard test). See [`t37c_forward_replay_stepper_plan.md`](t37c_forward_replay_stepper_plan.md) + SPEC V.55/V.56/V.57/B.28. The interaction model (§"the spine"), `CombatSession` seam (§3.1), harness (§3.4), and node-type decisions stay valid.
>
