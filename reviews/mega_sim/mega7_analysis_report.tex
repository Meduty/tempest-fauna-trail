...
\toprule
\textbf{name} & \textbf{affinity} & \textbf{role} & \textbf{tier} & \textbf{level} & \textbf{win\_rate} & \textbf{wr\_delta} \\
\midrule
...
The under-tuned list is dominated by \textbf{high-tier hybrids/mages that win on raw power but miss their budget} (Aurion, Hierarch) and \textbf{snow pieces} (Riven Frost-Wyrm, Cold-Iron Yeti, Hoarfrost Owl) --- reinforcing the snow-affinity anomaly from \S{}6. The over-tuned list is led by a T1 mage (Lostlight Wisp) and the rain/thunder hybrid Tyrants.\\
