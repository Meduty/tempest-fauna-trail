# 12_mega7_plots.R — headline figures for the mega8 (results/mega/mega8) report.
# Base R graphics only. Run after 11_mega7.R. Writes plots/m8_*.png.
...
C  <- readRDS(file.path(OUTDIR,"cache_mega8.rds"))
R8 <- C$R8; CB <- C$CB; ORD <- C$ORD8_pres; SIZE <- C$SIZE8[ORD]
px <- function(f) file.path(PLOTS, f)
mn <- function(x) mean(x, na.rm=TRUE)

# 9-role palette for the re-axed mega8 roster (mega7's warrior/hybrid are gone;
...
png(px("m8_01_role_wrdelta_vs_size.png"), 1050, 660, res=120)
par(mar=c(4,4,3,7))
rd <- C$role_wd8; cols <- ROLE_COLS[rownames(rd)]
matplot(sizes, t(rd), type="o", pch=19, lty=1, lwd=2, col=cols, xaxt="n",
        xlab="team size", ylab="wr_delta (tuning residual)",
        main="Role tuning residuals by team size (mega8)",
...
png(px("m8_02_variance_timeout_vs_size.png"), 1000, 600, res=120)
par(mar=c(4,4.5,3,4.5))
plot(C$bal8$size, C$bal8$sd_wr, type="o", pch=19, col="#1f77b4", lwd=2,
     xlab="team size", ylab="sd(per-piece win_rate)", ylim=c(0,0.30),
     main="Win-rate spread and timeout rate vs team size (mega8)")
par(new=TRUE)
plot(C$bal8$size, C$bal8$timeout, type="o", pch=17, col="#d62728", lwd=2,
...
# ---- 3. MAGE DEFICIT by size: mega7 vs mega8 --------------------------------
png(px("m8_03_mage_deficit_by_size.png"), 1000, 620, res=120)
par(mar=c(4,4,3,1))
wtd8 <- C$mt8$within_tier_def
R7 <- C$R7
wtdf <- function(d) {
...
m7sizes <- c("2v2"=2,"3v3"=3,"4v4"=4,"5v5"=5,"6v6"=6,"7v7"=7,"8v8"=8,"9v9"=9,"10v10"=10)
wtd7 <- if(!is.null(R7)) sapply(names(m7sizes), function(s) wtdf(R7[as.character(R7$stage)==s,])) else NULL
plot(sizes, wtd8, type="o", pch=19, col="#d62728", lwd=2,
     xlab="team size", ylab="within-tier mage deficit (non-mage − mage wr)",
     main="Mage deficit by team size (mega7 vs mega8)", xaxt="n",
     ylim=range(c(wtd8, wtd7, 0), na.rm=TRUE)+c(-0.01,0.01))
axis(1, sizes, ORD)
if(!is.null(wtd7)) { lines(m7sizes, wtd7, type="o", pch=4, col="grey40", lty=3, lwd=2) }
abline(h=0, lty=2, col="grey60")
legend("topright", c("mega8","mega7"), col=c("#d62728","grey40"),
       pch=c(19,4), lwd=2, lty=c(1,3), bty="n")
dev.off()

# ---- 4. ROLE WIN-RATE HEATMAP by size --------------------------------------
png(px("m8_04_role_wr_heatmap.png"), 1100, 520, res=120)
par(mar=c(5,6,3,5))
wr <- C$role_wr8; nr <- nrow(wr); nc <- ncol(wr)
image(1:nc, 1:nr, t(wr), col=colorRampPalette(c("#d73027","#fee08b","#1a9850"))(64),
      axes=FALSE, xlab="team size", ylab="", zlim=c(0.3,0.7),
      main="Role win_rate by size (mega8)")
...
png(px("m8_05_level_effect.png"), 1100, 560, res=120)
par(mfrow=c(1,2), mar=c(4,4,3,1))
lv_wr <- sapply(ORD, function(s){d<-R8[R8$stage==s,];sapply(1:3,function(l)mn(d$win_rate[d$level==l]))})
lv_wd <- sapply(ORD, function(s){d<-R8[R8$stage==s,];sapply(1:3,function(l)mn(d$wr_delta[d$level==l]))})
...
png(px("m8_06_weather_effects.png"), 1100, 560, res=120)
par(mfrow=c(1,2), mar=c(4,4,3,1))
rng <- range(c(C$wx8$own_minus_counter, C$wx8$own_minus_clear, 0), na.rm=TRUE)
plot(C$wx8$size, C$wx8$own_minus_counter, type="o", pch=19, col="#1f77b4", lwd=2,
     xlab="team size", ylab="win-rate advantage", xaxt="n",
     main="Own-weather advantage by size (RAW)", ylim=rng+c(-0.01,0.02))
axis(1, C$wx8$size)
points(C$wx8$size, C$wx8$own_minus_clear, type="o", pch=15, col="#2ca02c", lwd=2)
abline(h=0,lty=2,col="grey50")
legend("topright", c("own − counter","own − clear(baseline)"),
       col=c("#1f77b4","#2ca02c"), lwd=2, pch=c(19,15), bty="n", cex=.85)
aff8 <- C$aff8
bp <- barplot(aff8$own_minus_counter, names.arg=aff8$affinity, col=WX_COLS[aff8$affinity],
              ylim=range(c(aff8$own_minus_counter, aff8$own_minus_clear,0))+c(-0.01,0.02),
              ylab="own − counter wr", main="Per-affinity own-weather edge (RAW, pooled)")
abline(h=0); text(bp, aff8$own_minus_counter+0.003,
                  sprintf("%.3f",aff8$own_minus_counter), cex=0.7)
dev.off()

# ---- 7. OUTLIER SCATTER: wr_delta vs win_rate (combined) -------------------
png(px("m8_07_outlier_scatter.png"), 1050, 680, res=120)
par(mar=c(4,4,3,1))
...
plot(CB$win_rate, CB$wr_delta, col=adjustcolor(rcol,alpha.f=0.6), pch=19, cex=0.8,
     ...
     main="Piece outliers: win_rate vs wr_delta (mega8 combined, all stages+wx)")
...
png(px("m8_08_mage_wrdelta.png"), 1100, 900, res=120)
par(mar=c(4,9,3,2))
...
plot(mc$wr_delta, y, pch=19, col=cols, yaxt="n", xlab="wr_delta (level-pooled)",
     ylab="", main="Mage wr_delta per piece (mega8 combined)",
...
png(px("m8_09_wrdelta_boxplots.png"), 1200, 560, res=120)
...
boxplot(wr_delta~factor(role, levels=rd_ord), data=CB, col=bcol, las=2,
        ...
        main="wr_delta distribution by role (mega8 combined)")
...
png(px("m8_10_wrdelta_distribution.png"), 1200, 560, res=120)
par(mfrow=c(1,2), mar=c(4,4,3,1))
wd <- CB$wr_delta
...
if (!is.null(sp) && nrow(sp)) {
  ...
  png(px("m8_11_spread_outliers.png"), 1100, 900, res=120)
  par(mar=c(4,11,3,2))
  y <- seq_len(nrow(top))
  ...
  plot(top$excess_sd, y, pch=19, col=cols, yaxt="n",
       ...
       main="Most context-volatile pieces (mega8)",
...
png(px("m8_12_wr_vs_power.png"), 1250, 560, res=120)
par(mfrow=c(1,2), mar=c(5,4,3,1))
...
boxplot(win_rate~pbin, data=pdat, at=at, col="#c6dbef", xaxt="n",
        ...
        main="win_rate vs power, with expected_wr curve (mega8 combined)")
...
cat("[plots] wrote m8_01..m8_12 to", PLOTS, "\n")
