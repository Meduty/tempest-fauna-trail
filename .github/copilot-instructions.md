...
## Canonical documents (the four sources of truth)

| Doc | Role |
|---|---|
| [SPEC.md](../SPEC.md) | **The contract** — §G goal, §C context, §I interfaces, §V invariants, §T tasks, §B bugs, §D deferred. Wins on any conflict. Mutated **only** via the `/spec` workflow. |
| [ARCHITECTURE.md](../ARCHITECTURE.md) | **The system map** — how every system works + interacts, and where each lives in the code. Start here to navigate. |
| [CLAUDE.md](../CLAUDE.md) | Agent onboarding — structure, conventions, workflow. |
| [docs/](../docs/) | Per-system design docs (`docs/design/`), per-task plans (`docs/design/tasks/`), and the dev journal (`docs/journal/`). |
...
3. **The task plan doc** — `docs/design/tasks/tN_*_plan.md` (write one first if absent).
4. **Every design doc the task touches** — `docs/design/{systems,content}/*.md` in scope.
5. **Every piece of code the change touches** — read the modules + integration touch
...
- Update the affected design docs.
