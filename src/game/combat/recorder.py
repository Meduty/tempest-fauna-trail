...
class BattleResultRecorder:
    ...
    def record_cast(self, actor_id: str, target_id: str, tick: int, amount: int, damage_type: str, is_crit: bool = False) -> None:
        """Record a cast event with damage (called by loop for legacy-compatible cast recording)."""
        self._events.append(BattleEvent(
            tick=tick,
            actor_id=actor_id,
            target_id=target_id,
        ...
        self._damage_dealt[actor_id] = self._damage_dealt.get(actor_id, 0) + amount
        self._damage_taken[target_id] = self._damage_taken.get(target_id, 0) + amount

    def record_attack(self, actor_id: str, target_id: str, tick: int, amount: int, damage_type: str, is_crit: bool = False) -> None:
        """Record an attack event with damage (called by loop for legacy-compatible recording)."""
        self._events.append(BattleEvent(
            tick=tick,
            actor_id=actor_id,
            target_id=target_id,
    ...
    def _on_attack_landed(self, ctx: Any, event: AttackEvent) -> None:
        ...
        # This is used when attacks go through ctx.trigger_basic_attack from abilities
        # The main loop records attacks directly via record_attack for legacy compat
        pass

    def _on_damage_dealt(self, ctx: Any, event: DamageEvent) -> None:
        """Track damage for ability-driven damage not recorded elsewhere."""
        # Damage tracking for ability-framework driven damage
        # is handled via record_attack/record_cast for legacy compat
        pass
