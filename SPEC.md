...
**WIP (🟡 — MVP run-loop slice, `t10_mvp_run_loop_plan.md`):** the menu-to-menu playable loop over the finished backend — ~~T.10 RunStart~~ ✅ → ~~T.11 Trail~~ ✅ → **T.23a** Prep (full economy + placement, no items) → **T.15** reward/wiring + autosave/Continue → **T.13** run-summary BarChart → **T.23b** Prep items (trails — loop closes without it). Remaining order: T.23a → T.15(reward) → T.13 → T.15(wire) → T.23b. Guards V.63–V.68.
...
**Then — UI phase:** T.9 → **T.37a → T.37b → T.37c → T.12a → T.12b → T.12c** (combat view ✅) → **MVP run-loop slice: T.10 → T.11 → T.23a → T.15(reward) → T.13 → T.15(wire) → T.23b** (`t10_mvp_run_loop_plan.md`, `t23_prep_view_plan.md`); polish T.34a/b/c (ability tooltips) → **T.35a (Magnitude family) → T.35b (dead-stat balance)** + T.17.
...
T.8 → T.9 → T.37a → T.37b → T.37c → T.12a → T.12b → T.12c → **MVP run-loop slice:** T.10 → T.11 → T.23a → T.15(reward) → T.13 → T.15(wire) → T.23b (`t10_mvp_run_loop_plan.md`, `t23_prep_view_plan.md`)
