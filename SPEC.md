...
1. **T.29-pre** combat stat substrate — weather→`source:`-tagged modifiers (V.42), `compute_stat` single-fold + resource resync (V.43), anti-runaway snapshot (V.44), `source:` prefix vocab + `stat_breakdown` (V.45); `attack_speed`→float, drop `milli_AS` (amends V.34, B.18). Plan ready (`docs/design/tasks/t29_item_engine_plan.md` Part B). ← **next action: `/build T.29-pre`**
2. **T.29a** item engine core — components, 16-item cut, 3-slot equip, REWARD drops. Plan ready (`docs/design/tasks/t29_item_engine_plan.md`).
3. **T.29b** items rest — remaining 20 combined + 6 emblems + 6 special run-actions + interactive `sim_run` driver.
4. **T.31** augments — ~50 catalog, `RunModifiers` seam, `sim_run` augment policies; carries 3 paired Primordial-unlock RUN-augments + Primordial @1 signatures + @3 tier-up (D.20).
...
T.22 (economy + shop) → T.28a → T.28b → T.28c → T.28d (traits) → T.29-pre (combat stat substrate: weather→modifiers, AS→float/drop milli_AS) → T.29a → T.29b (items) → T.31 (augments)
