...
- V.69: **The run-loop applies a fought node's outcome only through `economy.apply_node_result(run, result)`.** The reward step feeds the resolved `BattleResult` to the single game-side orchestrator `apply_node_result(run, result) -> NodeResultSummary`, which **appends `result` to `Run.battle_log`**, grants **seeded** income (`apply_node_income`, win-bonus on a win only — `(run.seed, node_index)`, V.2) + fight tempest (`grant_fight_tempest`), then on a **win** marks the current node CLEARED + `advance_to_next_node` (→ `status = VICTORY` if last), else sets `status = DEFEAT` (a **DRAW** counts as non-win). The **producer** (run-loop wiring, never the view) calls it **exactly once per fight** and **never re-resolves** — the view already holds the resolved result (V.56/V.64). Because income/tempest are seeded, a Continue-after-load reproduces them byte-identically (V.2). **Commit-on-start:** every combat exit (end-panel Continue / control-bar Exit / Escape) applies the result — abandon/re-prep is rejected (it would save-scum a deterministic fight). Extends V.64 (result-out seam). (T.15a)
...
**WIP (🟡 — MVP run-loop slice, `t10_mvp_run_loop_plan.md`):** the menu-to-menu playable loop over the finished backend — ~~T.10 RunStart~~ ✅ → ~~T.11 Trail~~ ✅ → ~~T.23a Prep~~ ✅ (full economy + placement, no items) → **T.15a** reward step + result-out seam + autosave → **T.13** run-summary BarChart → **T.15b** terminal→Summary + Continue resume → **T.23b** Prep items (trails — loop closes without it). Remaining order: T.15a → T.13 → T.15b → T.23b. Guards V.63–V.69.
