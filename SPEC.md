...
| T.28d | Subsystem-heavy + currently-dormant trait effects — 6 **Primordial @1** signature kits (augment-gated, unreachable until T.31); 6 **affinity @10 apex** riders (crit-arc/slow-attackers/ability-haste-pool/untargetable/burst-reduction); **Scaled** @5 CC-immunity + @8 weather-as-buff override; **Spirit @8** echo-potency-reduction + pierce-untargetable-gate + ability-haste pool; Primordial @3 aspirational tier-up; all RNG-free | `game/combat/engine.py`, `game/combat/context.py`, `game/traits/`, `game/weather_effects.py`, `docs/design/tasks/t28_trait_effects_plan.md` | T.28c | M-L | 📋 Plan |
...
- D.10 Champion / enemy archetypes: the ~6-8 role archetypes and their `P = 1`
  ...
  `role_code` descriptor derive identity deterministically (V.31/V.32/V.33). Open
  remainder: per-archetype `P=1` base-stat tuning + enemy power tags.
...
T.22 (economy + shop) → T.28a → T.28b → T.28c → T.28d (traits) → T.29a → T.29b (items) → T.31 (augments)
