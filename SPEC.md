...
| T.36b | Unified roster axis-distribution solve (folds in the old T.36b grid + T.36c role pass — plan §13) — **optimize the axis marginals directly** (role is a pure fn of axes, V.32; stop fighting two grids). One solve over target marginals (stat 22/22/16, playstyle 24/24/12, reach 30/30 melee/ranged, **durability 11/8/13/28** [fixes the hybrid-35/arm-3 skew], intent ~26/22/12) + **soft role floors** (all roles ≥4: assassin/spellblade/spellslinger 4, bruiser 6, marksman 5, mage 6, swash 6, tank/support 8; caps tank ≤12 support ≤11), kings + 3 flip kits PINNED → **emergent role distro** (tank 12 / support 11 / swash 7 / mage 7 / bruiser 6 / marksman 5 / assassin 4 / spellblade 4 / spellslinger 4), **~20 curated re-axes** (identity-curated, not raw solver output); **new role `Spellslinger`** (`ranged`+playstyle-`hybrid`+`damage` — amends V.32); `marsh_thrush` utility-INT/damage-STR; **fit Callings→playstyle** + gentle Kinship rebalance (Beast 14→~10); rebuild moved-piece kits; **enemies parallel pass** (freer — no grid contract); **soft axis-marginal guard** (self-documenting, not §V); snapshot + role-matrix regen + `stat_edge` read | `game/content.py`, `game/abilities/champions.py`, `game/abilities/enemies.py`, `tests/game/test_content.py`, `docs/design/tasks/t32_role_matrix.txt`, `tests/game/test_role_intent.py`, `docs/design/tasks/t36_roster_stat_playstyle_rebalance_plan.md` | T.36a | L | 📋 Plan |
...
**Independent now (post-T.34, no UI dep):** T.35a (#42 Finding A — Magnitude-family refactor, byte-identical) can run any time after T.34c; T.35b (#42 Finding B — balance re-tune) after T.35a. **T.36a (Primordial diversification) → T.36b (unified roster axis-distribution solve)** run after T.35b — pure content/classification re-axis + kit rewrites; no UI/engine dep (new `Spellslinger` role + V.47-guard extension only). T.36b optimizes axis marginals directly (role derives, V.32), pins kings+flips, ~20 curated re-axes + trait-fit + kit rebuilds (champ + enemy passes). (Old T.36c role-reconciliation folded into T.36b — plan §13.)
