...
- D.15 Shop: **LOCKED — implemented in T.22; tier-gating revised to rank (V.74).**
  Lives in the Prep view. 5 champion slots, auto-refreshed each node entry (free).
  Manual reroll costs 1 Amber; the first reroll each node is free (counter resets
  every node advance). **Rank-gated tier probabilities** (`run.tempest_rank` 1-10,
  ...
  authored in `docs/design/tasks/t22_meta_progression_plan.md` and
  `shop.RANK_TIER_WEIGHTS`.
