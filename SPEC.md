...
| T.12c | Combat-view per-ability-shape VFX — **record the ability's targeting footprint** (reuse the handler's hit-determination: `enemies_in_radius`/`allies_in_radius`/`neighbors_of` → `circle`, `line_targets` → `line`) via `ctx.note_footprint` → `on_footprint` → recorder `footprint` records on `BattleResult` (**observer-only**, scoped to `current_cast_id`, sims byte-identical — V.61/V.2/V.54); view **draws + animates (expand/fade)** circle/line in the ability's element colour (`AbilityMeta` tags), single-target keeps swoosh/arrow. **Phase B (partial):** intent recolour landed — `classify_intent` (`ui/combat_playback.py`, from `AbilityMeta.tags`: heal→summon→damage-element→buff + `control` flag) recolours the recorded footprint shape: ally **heal/buff** → green halo (`SUCCESS`), **control** → `WARNING` telegraph ring outside the AoE (keyed `fp-tel-{cast_id}-{i}`). **Remaining:** status-apply flash + ally halos on heals that produce **no** targeting footprint. Sprites still deferred. (Landed via reconciliation of two parallel T.12c builds — remote VFX+hex+V.62 base + ported local intent layer.) Built headless → **user visual gates** | `game/targeting.py`, `game/combat/context.py`, `game/events.py`, `game/combat/recorder.py`, `game/models.py`, `ui/combat_playback.py`, `ui/views/combat.py`, `tests/game/test_combat.py`, `tests/ui/test_combat_playback.py`, `docs/design/tasks/t12c_combat_view_vfx_plan.md`, `docs/live/systems/ui.md` | T.12b, T.20, T.34 | M | 🔶 Partial |
...
| T.23 | Prep formation snapshot integration — lock player board placement in Prep, validate deployment constraints, pass explicit coordinates into combat init. **Engine primitive landed (V.62):** `build_combat(…, positions)` overrides `assign_spawns` (both sides, on-board + no-dup-cell validated, byte-identical when `None`) + `CombatSession.positions`; T.23 remaining work = the **Prep-side player-team `team_positions`** authoritative wrapper (deployment-zone + roster-id validation, team-only) over that primitive | `ui/views/prep.py`, `game/models.py`, `game/combat/`, `docs/design/tasks/t23_prep_formation_snapshot_plan.md` | T.1, T.3, T.15 | M | 📋 Plan |
