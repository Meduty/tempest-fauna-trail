...
**Independent now (post-T.34, no UI dep):** T.35a (#42 Finding A — Magnitude-family refactor, byte-identical) can run any time after T.34c; T.35b (#42 Finding B — balance re-tune) after T.35a. **T.36a (Primordial diversification) → T.36b (roster distribution re-axis)** run after T.35b — pure content/classification re-axis + kit rewrites; no UI/engine dep (new role + V.47-guard extension only).
