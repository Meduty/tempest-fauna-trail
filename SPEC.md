...
| T.36a | Primordial diversification — re-axis + kit-rewrite the **6 T10 kings** off uniform `hybrid/hybrid` into 6 distinct apex archetypes, **Calling-honest** (cast-Callings→`ability`, auto-Callings→`auto`): **Aurion** keeps hybrid/hybrid (*Ascendance* cast-ramp max 8 stacks), **Nerei** int/ability (*Grudge of the Flood* — `nerei_grudge` marker + `on_damage_pre` amp), **Borealis** hybrid/ability (Blizzard INT 2.28→2.7, frozen +15%), **Umbra** str/auto (every-5th-auto `STR·1.5`, no INT), **Mournhollow** str/ability (*Haunting Mist* + `grief` DoT `STR·0.4`/tick, parity coeff), **Aerion** hybrid/auto (every-3rd-auto chain + `attack_speed` steroid); **extend the V.47 guard to enforce `hybrid`→both STR+INT** (was INT-only, B.24) + dead-STR-hybrid detector test; fix stale `0.2·INT` comment (`test_content.py:363`→`0.25`); new `grief` DoT StatusDef + `nerei_grudge` marker; all kits deterministic (V.2/V.14), existing primitives only | `game/content.py`, `game/abilities/champions.py`, `game/status.py`, `tests/game/test_content.py`, `tests/game/test_role_intent.py`, `docs/design/tasks/t36_roster_stat_playstyle_rebalance_plan.md` | T.32, T.35a, T.35b | M | 📋 Plan |
| T.36b | Roster distribution re-axis — Calling-honest re-axis + kit-rewrite of **12 non-king champs** to land the full **22/22/16** target grid (preserves the destination cell multiset; honors Calling→playstyle); **3 minimal Calling tweaks** (`snowpelt_cub`+Bruiser, `granite_gorilla`+Bruiser, `eclipse_jaguar` restore Stalker [doc/code drift fix]); **new role `Spellslinger`** (`ranged`+playstyle-`hybrid`+`damage` — amends V.32), 3 occupants via grid-neutral swaps (`tempest_eel` int/hybrid; `voltmane_jackal` hybrid/hybrid+intent=damage ↔ `glacierback_mammoth`→hybrid/auto; `torrent_heron` hybrid/hybrid ↔ `goldhide_rhino`→hybrid/ability); `marsh_thrush` utility-INT/damage-STR (Galecrash `INT·6.55`→discounted `STR`, V.47-hybrid); str/ability coeffs discounted per parity (`mirewarden_toad`/`hollow_elk` `INT·3.3-3.9`→`STR·~1`); **self-documenting distribution guard** (soft, not §V); snapshot + role-matrix regen + `stat_edge` read | `game/content.py`, `game/abilities/champions.py`, `tests/game/test_content.py`, `docs/design/tasks/t32_role_matrix.txt`, `tests/game/test_role_intent.py`, `docs/design/tasks/t36_roster_stat_playstyle_rebalance_plan.md` | T.36a | L | 📋 Plan |
| T.36c | Role + trait distribution reconciliation — gently fix the lopsided **role** distribution (over: support/tank/swashbuckler; empty: assassin/bruiser; thin: spellblade/spellslinger) toward a soft target (champ 8/10/8/8/7/6/4/5/4 tank/support/swash/mage/marksman/bruiser/assassin/spellblade/spellslinger; enemy parallel) **without touching the stat×playstyle grid** — the grid-neutral levers are `intent`/`durability` (+ a few `reach`/playstyle swaps); **fit Callings to the resulting playstyle** (Calling→playstyle) + gentle Kinship rebalance (Beast 14→~10); rebuild moved-piece kits; enemies too (freer — no grid contract). **Assassin is the hard fill** (no natural squishy-melee-caster; via swash↔mage playstyle swaps or tank→squishy). Snapshot + role-matrix regen + distribution-guard update | `game/content.py`, `game/abilities/champions.py`, `game/abilities/enemies.py`, `tests/game/test_content.py`, `docs/design/tasks/t32_role_matrix.txt`, `tests/game/test_role_intent.py`, `docs/design/tasks/t36_roster_stat_playstyle_rebalance_plan.md` | T.36b | L | 📋 Plan |
...
**Independent now (post-T.34, no UI dep):** T.35a (#42 Finding A — Magnitude-family refactor, byte-identical) can run any time after T.34c; T.35b (#42 Finding B — balance re-tune) after T.35a. **T.36a (Primordial diversification) → T.36b (roster distribution re-axis) → T.36c (role+trait distribution reconciliation)** run after T.35b — pure content/classification re-axis + kit rewrites; no UI/engine dep (new role + V.47-guard extension only). T.36c is grid-neutral (intent/durability levers) + trait re-fit + kit rebuilds for moved pieces.
