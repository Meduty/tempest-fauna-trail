...
**WIP (🟡 — MVP run-loop slice, `t10_mvp_run_loop_plan.md`):** the menu-to-menu playable loop over the finished backend — ~~T.10 RunStart~~ ✅ → ~~T.11 Trail~~ ✅ → **T.23** Prep (full economy + placement) → **T.15** reward/wiring + autosave/Continue → **T.13** run-summary BarChart. Remaining order: T.23 → T.15(reward) → T.13 → T.15(wire). Guards V.63–V.66.
