...
- V.65: **Every run autosave goes through `save.save_run`.** Node-boundary autosaves + Save&Exit serialize the `Run` **only** via `game/save.py`'s atomic `save_run` (temp→fsync→`os.replace`, V.36) — no ad-hoc `json`/`to_dict` writes in views. Readers never observe a partial save; Continue loads via `load_run`. (T.15)
- V.66: **The Trail-owned weather `Refresher` is lifecycle-bounded.** Trail starts the T.7 `Refresher` on open and **stops it on pop / Save&Exit** (no leaked worker threads across views); all weather HTTP stays on the refresher's worker thread, never the Flet main thread (re-asserts V.4 at the UI seam). Missing/`UNKNOWN` cache entries render the city `default_weather` placeholder — the view never blocks on a fetch. (T.11)
...
| T.11 | Trail view + route-map Canvas — `viz/route_map.py` node-line (all nodes, state-tint cleared/current/upcoming, weather icon, boss-marked, hit-test via overlay) + node focus panel (weather-favor + affinity-clash + enemy preview) + team summary; **live weather via the T.7 `Refresher`/`WeatherCache`** (Trail-owned, lifecycle-bounded, off-main-thread, `default_weather` fallback — V.66/V.4); Play-Next → Prep, Save&Exit autosaves | `viz/route_map.py`, `ui/views/trail.py`, `tests/viz/test_route_map.py`, `docs/live/systems/ui.md`, `docs/design/tasks/t10_mvp_run_loop_plan.md` | T.4, T.6, T.7, T.8 | L | 🟡 WIP |
...
| T.13 | Run summary view — run-end (`VICTORY`/`DEFEAT`) `ft.BarChart` of **damage-per-battle** from `run.battle_log` (graded viz, kept — no shortcut) + outcome banner + nodes-cleared/final-Amber/rank; return-to-menu. Pure builder `viz/run_summary.py` (asserts data, not pixels) | `viz/run_summary.py`, `ui/views/summary.py`, `tests/viz/test_run_summary.py`, `docs/design/tasks/t10_mvp_run_loop_plan.md` | T.3, T.8 | M | 🟡 WIP |
| T.14 | Save/load — JSON serialization of Run state | `game/save.py` | T.1 | S | ✅ Done |
| T.15 | Routing + app wiring — `main.py` `page.views` stack menu→run_start→trail⇄prep→combat→reward→(trail\|summary)→menu; **combat-result-out seam** (`on_exit(result)`, V.64) + reward step (`apply_node_income`/`grant_fight_tempest`/mark-cleared/advance); **autosave each node-boundary + Save&Exit via `save.save_run`** (V.65); **Continue** = `load_run`→Trail; `_pop`/`on_view_pop` unwind stops combat autoplay + Trail refresher (V.66) | `main.py`, `ui/views/reward.py`, `ui/views/combat.py`, `docs/design/tasks/t10_mvp_run_loop_plan.md` | T.9, T.10, T.11, T.13, T.14, T.23 | M | 🟡 WIP |
...
| T.23 | Prep view (full economy) + team-positions wrapper — `ui/views/prep.py`: hex placement (drag/Auto-Place/Reset within the **allied deployment zone** `q < BOARD_WIDTH//2`), bench↔field, **shop** (`buy_from_shop`/`reroll_shop`/`sell_champion`/`take_supply_champion`), items, stat tooltips (reuse `render_for`); Start-Combat builds a `CombatSession` (shape-identical to the dev-harness producer) with `positions=team_positions`. **Engine primitive already landed (V.62)** — T.23's only engine-side residue is the **team-only validated wrapper** `validate_team_positions` (deployment-zone + roster-id checks) in `game/loadout.py` over `build_combat`. (Supersedes the pre-V.62 `t23_*_plan.md`, which cited the removed `game/combat.py` + `resolve_combat(team_positions=)`.) | `ui/views/prep.py`, `game/loadout.py`, `tests/game/test_prep_positions.py`, `docs/live/systems/ui.md`, `docs/design/tasks/t10_mvp_run_loop_plan.md` | T.1, T.3, T.10, T.11, T.22 | M | 🟡 WIP |
...
- D.28 Combat-view polish — **DEFERRED (post-MVP).** The combat view (T.12/T.12c) is
  ...
  proper timeline/scheduler that interleaves moves→casts→attacks with dwell. (2) The
  **intra-tick stagger is manual-`Next` only** (`reveal_n` + `_drip_action_beats`,
  ...
  shape/number/lunge timing polish + sprite art still deferred (D.27). Owner: a
  future T.12d / T.12-polish task.
- D.29 Run-loop shell gap — **RESOLVED [2026-06-24] (T.10/T.11/T.13/T.15/T.23,
  ...
  the finished backend (economy/shop/save/encounter/combat all ✅). Guards V.63–V.66.
  **Still deferred from this slice (post-MVP):** (1) **augment-pick UI** — no
  augment-selection screen; `Run.active_augments` stays seeded/empty, combat honors any
  present via `RunModifiers` (owner: a future T.31-UI task); (2) **Trail/Prep visual
  polish** — views ship minimal-but-functional (node-line route, plain Prep board) per
  the "functional first" directive (D.27/D.28 combat-view polish unaffected).

## Implementation Order

### Current Status & Next Steps

LIVING snapshot — refresh via `/spec` whenever a §T status flips. Last: 2026-06-24, post-T.12c merge — MVP run-loop slice planned (`t10_mvp_run_loop_plan.md`).

**Done (✅):** T.1-T.9, T.12a/b/c, T.14, T.16, T.18-T.22, T.24-T.37c — engine, weather, route+content, economy/shop, full trait chain, ability catalog, role/scaling revamps, save/load, playtest CLI, menu + **combat view** (replay stepper + VFX). Backend headless-complete. T.17 docs 🔶.

**WIP (🟡 — MVP run-loop slice, `t10_mvp_run_loop_plan.md`):** the menu-to-menu playable loop over the finished backend — **T.10** RunStart → **T.11** Trail (route-map Canvas + live weather) → **T.23** Prep (full economy + placement) → **T.15** reward/wiring + autosave/Continue → **T.13** run-summary BarChart. Build order: T.10 → T.11 → T.23 → T.15(reward) → T.13 → T.15(wire). Guards V.63–V.66.
...
**Then — UI phase:** T.9 → **T.37a → T.37b → T.37c → T.12a → T.12b → T.12c** (combat view ✅) → **MVP run-loop slice: T.10 → T.11 → T.23 → T.15(reward) → T.13 → T.15(wire)** (`t10_mvp_run_loop_plan.md`); polish T.34a/b/c (ability tooltips) → **T.35a (Magnitude family) → T.35b (dead-stat balance)** + T.17.
...
T.8 → T.9 → T.37a → T.37b → T.37c → T.12a → T.12b → T.12c → **MVP run-loop slice:** T.10 → T.11 → T.23 → T.15(reward) → T.13 → T.15(wire) (`t10_mvp_run_loop_plan.md`)
