...
| T.28c | Trait combat primitives batch 2 + apex effects — echo/double-cast (Spirit/Channeler), mana-denial aura (Trickster), ability-splash (Mystic), Swarm on-death spawns, Hunter empowered-shot/pierce/cleave, Scaled weather-as-buff, Primordial kit hooks, Packmate `@full-board` apex; all RNG-free | `game/combat/engine.py`, `game/combat/context.py`, `game/abilities/`, `game/traits/`, `docs/design/tasks/t28_trait_effects_plan.md` | T.28b | M | 📋 Plan |
...
T.22 (economy + shop) → T.28a → T.28b → T.28c (traits) → T.29a → T.29b (items) → T.31 (augments)
