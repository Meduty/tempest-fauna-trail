...
| T.12d_b | Combat-view autoplay + queue + end summary — **fixed-cadence autoplay** (speed toggle 0.5×/1×/2×, each tick = one Next-step reusing the `_drip_action_beats` reveal that already paints FX, V.56); action queue **future-only** + next-highlight; per-champion **end-summary** overlay. **Deletes the B.35 dwell stopgap** (band-aid on the broken event-paced path); resolves D.28 (1) | `ui/views/combat.py`, `ui/combat_playback.py`, `docs/live/systems/ui.md`, `tests/ui/test_combat_playback.py` | T.12d_a | M | 📋 Plan |
...
- D.28 Combat-view polish — **DEFERRED (post-MVP).** The combat view (T.12/T.12c) is
  ...
  **[2026-06-29] Owned by §T T.12d (📋 Plan, `t12d_combat_view_rework_plan.md`):**
  **T.12d_b** closes (1) (fixed-cadence autoplay reusing `_drip_action_beats`, deleting
  the B.35 dwell band-aid); **T.12d_a** closes (2) (shared infocard + the cadence path
  applies the stagger to autoplay too). Mark (1)+(2) **RESOLVED on build**; (3) sprite
  art / full timeline-scheduler stays deferred (a possible later T.12e).
